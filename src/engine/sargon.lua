...
function ATKSAV()
    ...
    local l = #iif(wh, ATKLSTW, ATKLSTB);
...
function FNDMOV()
    ...
    while (NPLY >= 0) do
        ...
        while (#MLIST > t and MLIST[1 + PLYIX[1 + MLPTRI]].MLPTR == MLIST[1 + t].MLPTR) do
...
function CPTRMV()
    ...
    if (BESTM >= 0) then
        ...
        print("bestmove: " .. move .. ", nodes of moves: " .. string.format("%d", nodes) .. ", calc.time: " ..
...
--
function sectime()
    return (math.floor(playdate.getCurrentTimeMilliseconds()));
end
...
function PgnMove(idx)
    ...
    if string.len(movestr) == 0 then
        movestr = pc .. string.sub(Ob.move, 1, 2) .. capt .. string.sub(Ob.move, 4, 5) .. iif(fl.f5 > 0, "=q", "") ..
                      iif(PMATE > 0, "x", iif(CKFLG > 0, "+", ""));
    end

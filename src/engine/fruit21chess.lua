...
--  TODO TRENT get change to playdate file stuff
UseMcache = false; -- to use files for faster performance (saves some memory variables),
-- set "false" for normal lua, needed for very slow lua case
TRUE = 1;
FALSE = 0;

UseTable = true; -- const bool
MaterialTableSize = 64 * 8; -- const size of Material hashing (array elements to use)
PawnTableSize = 64 * 8; -- const size of Pawn hashing (array elements to use)

UseTrans = true; -- const bool
TransSize = 64 * 8; -- const size of transp-table (array elements to use)
...
PieceFrom12 = {WhitePawn256, BlackPawn256, WhiteKnight256, BlackKnight256, WhiteBishop256, BlackBishop256, WhiteRook256,
               BlackRook256, WhiteQueen256, BlackQueen256, WhiteKing256, BlackKing256}; -- const int[12]
...
SquareFrom64 = {A1, B1, C1, D1, E1, F1, G1, H1, A2, B2, C2, D2, E2, F2, G2, H2, A3, B3, C3, D3, E3, F3, G3, H3, A4, B4,
                C4, D4, E4, F4, G4, H4, A5, B5, C5, D5, E5, F5, G5, H5, A6, B6, C6, D6, E6, F6, G6, H6, A7, B7, C7, D7,
                E7, F7, G7, H7, A8, B8, C8, D8, E8, F8, G8, H8}; -- const int[64]
...
function MOVE_MAKE_FLAGS(from, to, flags)
    return bit.bor(bit.lshift(SquareTo64[1 + from], 6), bit.bor(SquareTo64[1 + to], flags));
end
...
TotalPhase = (PawnPhase * 16) + (KnightPhase * 4) + (BishopPhase * 4) + RookPhase * 4 + (QueenPhase * 2); -- const int
...
function attack_init() -- void
    ...
    --    mFile = playdate.file.open("mem_dat1.txt", playdate.file.kFileRead)

    --    if(mfile==nil) then
    --    --   mfile = io.open("mem_dat1.txt", "w")   -- then create
    --      mFile = playdate.file.open("mem_dat1.txt", playdate.file.kFileWrite)
...
function board_is_ok(board) -- const bool
    ...
    for colour = 0, 1, 1 do
        ...
        for pos = 0, size - 1, 1 do
            ...
            if (pos ~= 0 and PieceOrder[1 + piece] >
                PieceOrder[1 + board.square[1 + board.piece[1 + colour][1 + pos - 1]]]) then
                return false;
            end
        end
    ...
    if (board.piece_nb ~= board.piece_size[1 + White] + board.pawn_size[1 + White] + board.piece_size[1 + Black] +
        board.pawn_size[1 + Black]) then
        return false;
    end
...
function board_copy(dst, src) -- void
    ...
    for i = 0, #src.piece[1 + 0] - 1, 1 do
        dst.piece[1 + 0][1 + i] = src.piece[1 + 0][1 + i];
    end
    for i = 0, #src.piece[1 + 1] - 1, 1 do
    ...
    for i = 0, #src.pawn_file[1 + 0] - 1, 1 do
        dst.pawn_file[1 + 0][1 + i] = src.pawn_file[1 + 0][1 + i];
    end
    for i = 0, #src.pawn_file[1 + 1] - 1, 1 do
    ...
    for colour = 0, 1, 1 do
        ...
        if (iDbg01) then
            ...
            for i = 0, board.piece_size[1 + colour] - 1, 1 do
                ...
                if (i == 0) then -- king
                    ...
                    ASSERT(59, PieceOrder[1 + board.square[1 + board.piece[1 + colour][1 + i]]] <=
                        PieceOrder[1 + board.square[1 + board.piece[1 + colour][1 + i - 1]]]);
                end
            end
        end
        ...
        for sq_64 = 0, 63, 1 do
            ...
            if (COLOUR_IS(piece, colour) and PIECE_IS_PAWN(piece)) then
                ...
                board.piece_nb = board.piece_nb + 1;
                board.number[1 + PieceTo12[1 + piece]] = board.number[1 + PieceTo12[1 + piece]] + 1;
                board.pawn_file[1 + colour][1 + SQUARE_FILE(sq)] = bit.bor(
                    board.pawn_file[1 + colour][1 + SQUARE_FILE(sq)], BitEQ[1 + PAWN_RANK(sq, colour)]);
            end
        end
    ...
    for colour = 0, 1, 1 do
        ...
        if (bit.band(mat_info.cflags[1 + me], MatBishopFlag) ~= 0) then
            ...
            if (pawn ~= SquareNone) then -- all pawns on one file
                ...
                if (SQUARE_FILE(king) == SQUARE_FILE(pawn) and PAWN_RANK(king, me) > PAWN_RANK(pawn, me) and
                    (not bishop_can_attack(board, king, me))) then
                    mul[1 + me] = 1; -- 1/16
                end
            end
        end
        ...
        while (true) do
            ...
            if (ptype == Rook64) then
                ...
                if (PAWN_RANK(from, me) == Rank7) then
                    ...
                    if (bit.band(pawn_info.flags[1 + opp], BackRankFlag) ~= 0 or PAWN_RANK(KING_POS(board, opp), me) ==
                        Rank8) then
    ...
    if (UseKingAttack) then
        ...
        for colour = 0, 1, 1 do
            ...
            if (bit.band(mat_info.cflags[1 + colour], MatKingFlag) ~= 0) then
                ...
                op[1 + colour] = op[1 + colour] - (attack_tot * KingAttackOpening * KingAttackWeight[1 + piece_nb]) /
                                     256;
            end
        end
    end
...
function eval_passer(board, pawn_info, opening, endgame) -- int
    ...
    for colour = 0, 1, 1 do
        ...
        while (true) do
            ...
            if (board.piece_size[1 + def] <= 1 and (unstoppable_passer(board, sq, att) or king_passer(board, sq, att))) then
...
function eval_pattern(board, opening, endgame) -- int
    ...
    if ((board.square[1 + A7] == WB and board.square[1 + B6] == BP) or
        (board.square[1 + B8] == WB and board.square[1 + C7] == BP)) then
        opening.v = opening.v - TrappedBishop;
        endgame.v = endgame.v - TrappedBishop;
    end

    if ((board.square[1 + H7] == WB and board.square[1 + G6] == BP) or
        (board.square[1 + G8] == WB and board.square[1 + F7] == BP)) then
        opening.v = opening.v - TrappedBishop;
        endgame.v = endgame.v - TrappedBishop;
    end

    if ((board.square[1 + A2] == BB and board.square[1 + B3] == WP) or
        (board.square[1 + B1] == BB and board.square[1 + C2] == WP)) then
        opening.v = opening.v + TrappedBishop;
        endgame.v = endgame.v + TrappedBishop;
    end

    if ((board.square[1 + H2] == BB and board.square[1 + G3] == WP) or
        (board.square[1 + G1] == BB and board.square[1 + F2] == WP)) then
        opening.v = opening.v + TrappedBishop;
        endgame.v = endgame.v + TrappedBishop;
    end
    ...
    if ((board.square[1 + C1] == WK or board.square[1 + B1] == WK) and
        (board.square[1 + A1] == WR or board.square[1 + A2] == WR or board.square[1 + B1] == WR)) then
        opening.v = opening.v - BlockedRook;
    end

    if ((board.square[1 + F1] == WK or board.square[1 + G1] == WK) and
        (board.square[1 + H1] == WR or board.square[1 + H2] == WR or board.square[1 + G1] == WR)) then
        opening.v = opening.v - BlockedRook;
    end

    if ((board.square[1 + C8] == BK or board.square[1 + B8] == BK) and
        (board.square[1 + A8] == BR or board.square[1 + A7] == BR or board.square[1 + B8] == BR)) then
        opening.v = opening.v + BlockedRook;
    end

    if ((board.square[1 + F8] == BK or board.square[1 + G8] == BK) and
        (board.square[1 + H8] == BR or board.square[1 + H7] == BR or board.square[1 + G8] == BR)) then
        opening.v = opening.v + BlockedRook;
    end
end
...
function king_passer(board, pawn, colour) -- bool
    ...
    if (DISTANCE(king, prom) <= 1 and DISTANCE(king, pawn) <= 1 and
        (SQUARE_FILE(king) ~= file or (file ~= FileA and file ~= FileH))) then
        return true;
    end
...
function board_from_fen(board, fenstr) -- void
    ...
    if (c == "-") then -- no en-passant
        ...
        if (board.square[1 + sq] ~= Empty or board.square[1 + pawn] ~= PawnMake[1 + COLOUR_OPP(board.turn)] or
            (board.square[1 + pawn - 1] ~= PawnMake[1 + board.turn] and board.square[1 + pawn + 1] ~=
                PawnMake[1 + board.turn])) then
            sq = SquareNone;
        end
    end
...
function add_quiet_checks(list, board) -- void
    ...
    if (board.square[1 + from] == pawn) then
        ...
        from = to - (2 * inc);
        if (board.square[1 + from] == pawn) then
            if (PAWN_RANK(from, me) == Rank2 and board.square[1 + to] == Empty and board.square[1 + from + inc] == Empty) then
...
function add_castle_checks(list, board) -- void
    ...
    if (COLOUR_IS_WHITE(board.turn)) then
        ...
        if (bit.band(board.flags, FlagsWhiteKingCastle) ~= 0 and board.square[1 + F1] == Empty and board.square[1 + G1] ==
            Empty and (not is_attacked(board, F1, Black))) then
            add_check(list, MOVE_MAKE_FLAGS(E1, G1, MoveCastle), board);
        end

        if (bit.band(board.flags, FlagsWhiteQueenCastle) ~= 0 and board.square[1 + D1] == Empty and board.square[1 + C1] ==
            Empty and board.square[1 + B1] == Empty and (not is_attacked(board, D1, Black))) then
            add_check(list, MOVE_MAKE_FLAGS(E1, C1, MoveCastle), board);
        end

    else -- black

        if (bit.band(board.flags, FlagsBlackKingCastle) ~= 0 and board.square[1 + F8] == Empty and board.square[1 + G8] ==
            Empty and (not is_attacked(board, F8, White))) then
            add_check(list, MOVE_MAKE_FLAGS(E8, G8, MoveCastle), board);
        end

        if (bit.band(board.flags, FlagsBlackQueenCastle) ~= 0 and board.square[1 + D8] == Empty and board.square[1 + C8] ==
            Empty and board.square[1 + B8] == Empty and (not is_attacked(board, D8, White))) then
...
function move_is_check(move, board) -- bool
    ...
    if (is_pinned(board, from, opp) and DELTA_INC_LINE(king - to) ~= DELTA_INC_LINE(king - from)) then
        return true;
    end
...
function move_is_pseudo(move, board) -- bool
    ...
    if (PIECE_IS_PAWN(piece)) then
        ...
        if (capture == Empty) then
            ...
            if (delta == (2 * inc) and PAWN_RANK(from, me) == Rank2 and board.square[1 + from + inc] == Empty) then
                return true;
            end
...
function option_get(var) -- string
    ...
    i = option_find(var);
    if (i == nil) then
        return my_fatal("option_get(): unknown option : " .. var .. "\n");
    end
            ...
            if (bit.band(t2, BitGT[1 + rank]) == 0) then
                ...
                if (bit.band(q, BitGT[1 + rank]) == 0) then
                    ...
                    if (n >= 0) then
                        ...
                        n = n -
                                BitCount[1 +
                                    bit.band(BitRev[1 + board.pawn_file[1 + opp][1 + file - 1]], BitEQ[1 + rank + 1])];
                        n = n -
                                BitCount[1 +
                                    bit.band(BitRev[1 + board.pawn_file[1 + opp][1 + file + 1]], BitEQ[1 + rank + 1])];
...
function random_init()
    ...
    Rn64("0x9D39247E33776D41");
    Rn64("0x2AF7398005AAA5C7");
    Rn64("0x44DB015024623547");
    Rn64("0x9C15F73E62A76AE2");
    Rn64("0x75834465489C0C89");
    Rn64("0x3290AC3A203001BF");
    Rn64("0x0FBBAD1F61042279");
    Rn64("0xE83A908FF2FB60CA");
    Rn64("0x0D7E765D58755C10");
    Rn64("0x1A083822CEAFE02D");
    Rn64("0x9605D5F0E25EC3B0");
    Rn64("0xD021FF5CD13A2ED5");
    Rn64("0x40BDF15D4A672E32");
    Rn64("0x011355146FD56395");
    Rn64("0x5DB4832046F3D9E5");
    Rn64("0x239F8B2D7FF719CC");
    Rn64("0x05D1A1AE85B49AA1");
    Rn64("0x679F848F6E8FC971");
    Rn64("0x7449BBFF801FED0B");
    Rn64("0x7D11CDB1C3B7ADF0");
    Rn64("0x82C7709E781EB7CC");
    Rn64("0xF3218F1C9510786C");
    Rn64("0x331478F3AF51BBE6");
    Rn64("0x4BB38DE5E7219443");
    Rn64("0xAA649C6EBCFD50FC");
    Rn64("0x8DBD98A352AFD40B");
    Rn64("0x87D2074B81D79217");
    Rn64("0x19F3C751D3E92AE1");
    Rn64("0xB4AB30F062B19ABF");
    Rn64("0x7B0500AC42047AC4");
    Rn64("0xC9452CA81A09D85D");
    Rn64("0x24AA6C514DA27500");
    Rn64("0x4C9F34427501B447");
    Rn64("0x14A68FD73C910841");
    Rn64("0xA71B9B83461CBD93");
    Rn64("0x03488B95B0F1850F");
    Rn64("0x637B2B34FF93C040");
    Rn64("0x09D1BC9A3DD90A94");
    Rn64("0x3575668334A1DD3B");
    Rn64("0x735E2B97A4C45A23");
    Rn64("0x18727070F1BD400B");
    Rn64("0x1FCBACD259BF02E7");
    Rn64("0xD310A7C2CE9B6555");
    Rn64("0xBF983FE0FE5D8244");
    Rn64("0x9F74D14F7454A824");
    Rn64("0x51EBDC4AB9BA3035");
    Rn64("0x5C82C505DB9AB0FA");
    Rn64("0xFCF7FE8A3430B241");
    Rn64("0x3253A729B9BA3DDE");
    Rn64("0x8C74C368081B3075");
    Rn64("0xB9BC6C87167C33E7");
    Rn64("0x7EF48F2B83024E20");
    Rn64("0x11D505D4C351BD7F");
    Rn64("0x6568FCA92C76A243");
    Rn64("0x4DE0B0F40F32A7B8");
    Rn64("0x96D693460CC37E5D");
    Rn64("0x42E240CB63689F2F");
    Rn64("0x6D2BDCDAE2919661");
    Rn64("0x42880B0236E4D951");
    Rn64("0x5F0F4A5898171BB6");
    Rn64("0x39F890F579F92F88");
    Rn64("0x93C5B5F47356388B");
    Rn64("0x63DC359D8D231B78");
    Rn64("0xEC16CA8AEA98AD76");
    Rn64("0x5355F900C2A82DC7");
    Rn64("0x07FB9F855A997142");
    Rn64("0x5093417AA8A7ED5E");
    Rn64("0x7BCBC38DA25A7F3C");
    Rn64("0x19FC8A768CF4B6D4");
    Rn64("0x637A7780DECFC0D9");
    Rn64("0x8249A47AEE0E41F7");
    Rn64("0x79AD695501E7D1E8");
    Rn64("0x14ACBAF4777D5776");
    Rn64("0xF145B6BECCDEA195");
    Rn64("0xDABF2AC8201752FC");
    Rn64("0x24C3C94DF9C8D3F6");
    Rn64("0xBB6E2924F03912EA");
    Rn64("0x0CE26C0B95C980D9");
    Rn64("0xA49CD132BFBF7CC4");
    Rn64("0xE99D662AF4243939");
    Rn64("0x27E6AD7891165C3F");
    Rn64("0x8535F040B9744FF1");
    Rn64("0x54B3F4FA5F40D873");
    Rn64("0x72B12C32127FED2B");
    Rn64("0xEE954D3C7B411F47");
    Rn64("0x9A85AC909A24EAA1");
    Rn64("0x70AC4CD9F04F21F5");
    Rn64("0xF9B89D3E99A075C2");
    Rn64("0x87B3E2B2B5C907B1");
    Rn64("0xA366E5B8C54F48B8");
    Rn64("0xAE4A9346CC3F7CF2");
    Rn64("0x1920C04D47267BBD");
    Rn64("0x87BF02C6B49E2AE9");
    Rn64("0x092237AC237F3859");
    Rn64("0xFF07F64EF8ED14D0");
    Rn64("0x8DE8DCA9F03CC54E");
    Rn64("0x9C1633264DB49C89");
    Rn64("0xB3F22C3D0B0B38ED");
    Rn64("0x390E5FB44D01144B");
    Rn64("0x5BFEA5B4712768E9");
    Rn64("0x1E1032911FA78984");
    Rn64("0x9A74ACB964E78CB3");
    Rn64("0x4F80F7A035DAFB04");
    Rn64("0x6304D09A0B3738C4");
    Rn64("0x2171E64683023A08");
    Rn64("0x5B9B63EB9CEFF80C");
    Rn64("0x506AACF489889342");
    Rn64("0x1881AFC9A3A701D6");
    Rn64("0x6503080440750644");
    Rn64("0xDFD395339CDBF4A7");
    Rn64("0xEF927DBCF00C20F2");
    Rn64("0x7B32F7D1E03680EC");
    Rn64("0xB9FD7620E7316243");
    Rn64("0x05A7E8A57DB91B77");
    Rn64("0xB5889C6E15630A75");
    Rn64("0x4A750A09CE9573F7");
    Rn64("0xCF464CEC899A2F8A");
    Rn64("0xF538639CE705B824");
    Rn64("0x3C79A0FF5580EF7F");
    Rn64("0xEDE6C87F8477609D");
    Rn64("0x799E81F05BC93F31");
    Rn64("0x86536B8CF3428A8C");
    Rn64("0x97D7374C60087B73");
    Rn64("0xA246637CFF328532");
    Rn64("0x043FCAE60CC0EBA0");
    Rn64("0x920E449535DD359E");
    Rn64("0x70EB093B15B290CC");
    Rn64("0x73A1921916591CBD");
    Rn64("0x56436C9FE1A1AA8D");
    Rn64("0xEFAC4B70633B8F81");
    Rn64("0xBB215798D45DF7AF");
    Rn64("0x45F20042F24F1768");
    Rn64("0x930F80F4E8EB7462");
    Rn64("0xFF6712FFCFD75EA1");
    Rn64("0xAE623FD67468AA70");
    Rn64("0xDD2C5BC84BC8D8FC");
    Rn64("0x7EED120D54CF2DD9");
    Rn64("0x22FE545401165F1C");
    Rn64("0xC91800E98FB99929");
    Rn64("0x808BD68E6AC10365");
    Rn64("0xDEC468145B7605F6");
    Rn64("0x1BEDE3A3AEF53302");
    Rn64("0x43539603D6C55602");
    Rn64("0xAA969B5C691CCB7A");
    Rn64("0xA87832D392EFEE56");
    Rn64("0x65942C7B3C7E11AE");
    Rn64("0xDED2D633CAD004F6");
    Rn64("0x21F08570F420E565");
    Rn64("0xB415938D7DA94E3C");
    Rn64("0x91B859E59ECB6350");
    Rn64("0x10CFF333E0ED804A");
    Rn64("0x28AED140BE0BB7DD");
    Rn64("0xC5CC1D89724FA456");
    Rn64("0x5648F680F11A2741");
    Rn64("0x2D255069F0B7DAB3");
    Rn64("0x9BC5A38EF729ABD4");
    Rn64("0xEF2F054308F6A2BC");
    Rn64("0xAF2042F5CC5C2858");
    Rn64("0x480412BAB7F5BE2A");
    Rn64("0xAEF3AF4A563DFE43");
    Rn64("0x19AFE59AE451497F");
    Rn64("0x52593803DFF1E840");
    Rn64("0xF4F076E65F2CE6F0");
    Rn64("0x11379625747D5AF3");
    Rn64("0xBCE5D2248682C115");
    Rn64("0x9DA4243DE836994F");
    Rn64("0x066F70B33FE09017");
    Rn64("0x4DC4DE189B671A1C");
    Rn64("0x51039AB7712457C3");
    Rn64("0xC07A3F80C31FB4B4");
    Rn64("0xB46EE9C5E64A6E7C");
    Rn64("0xB3819A42ABE61C87");
    Rn64("0x21A007933A522A20");
    Rn64("0x2DF16F761598AA4F");
    Rn64("0x763C4A1371B368FD");
    Rn64("0xF793C46702E086A0");
    Rn64("0xD7288E012AEB8D31");
    Rn64("0xDE336A2A4BC1C44B");
    Rn64("0x0BF692B38D079F23");
    Rn64("0x2C604A7A177326B3");
    Rn64("0x4850E73E03EB6064");
    Rn64("0xCFC447F1E53C8E1B");
    Rn64("0xB05CA3F564268D99");
    Rn64("0x9AE182C8BC9474E8");
    Rn64("0xA4FC4BD4FC5558CA");
    Rn64("0xE755178D58FC4E76");
    Rn64("0x69B97DB1A4C03DFE");
    Rn64("0xF9B5B7C4ACC67C96");
    Rn64("0xFC6A82D64B8655FB");
    Rn64("0x9C684CB6C4D24417");
    Rn64("0x8EC97D2917456ED0");
    Rn64("0x6703DF9D2924E97E");
    Rn64("0xC547F57E42A7444E");
    Rn64("0x78E37644E7CAD29E");
    Rn64("0xFE9A44E9362F05FA");
    Rn64("0x08BD35CC38336615");
    Rn64("0x9315E5EB3A129ACE");
    Rn64("0x94061B871E04DF75");
    Rn64("0xDF1D9F9D784BA010");
    Rn64("0x3BBA57B68871B59D");
    Rn64("0xD2B7ADEEDED1F73F");
    Rn64("0xF7A255D83BC373F8");
    Rn64("0xD7F4F2448C0CEB81");
    Rn64("0xD95BE88CD210FFA7");
    Rn64("0x336F52F8FF4728E7");
    Rn64("0xA74049DAC312AC71");
    Rn64("0xA2F61BB6E437FDB5");
    Rn64("0x4F2A5CB07F6A35B3");
    Rn64("0x87D380BDA5BF7859");
    Rn64("0x16B9F7E06C453A21");
    Rn64("0x7BA2484C8A0FD54E");
    Rn64("0xF3A678CAD9A2E38C");
    Rn64("0x39B0BF7DDE437BA2");
    Rn64("0xFCAF55C1BF8A4424");
    Rn64("0x18FCF680573FA594");
    Rn64("0x4C0563B89F495AC3");
    Rn64("0x40E087931A00930D");
    Rn64("0x8CFFA9412EB642C1");
    Rn64("0x68CA39053261169F");
    Rn64("0x7A1EE967D27579E2");
    Rn64("0x9D1D60E5076F5B6F");
    Rn64("0x3810E399B6F65BA2");
    Rn64("0x32095B6D4AB5F9B1");
    Rn64("0x35CAB62109DD038A");
    Rn64("0xA90B24499FCFAFB1");
    Rn64("0x77A225A07CC2C6BD");
    Rn64("0x513E5E634C70E331");
    Rn64("0x4361C0CA3F692F12");
    Rn64("0xD941ACA44B20A45B");
    Rn64("0x528F7C8602C5807B");
    Rn64("0x52AB92BEB9613989");
    Rn64("0x9D1DFA2EFC557F73");
    Rn64("0x722FF175F572C348");
    Rn64("0x1D1260A51107FE97");
    Rn64("0x7A249A57EC0C9BA2");
    Rn64("0x04208FE9E8F7F2D6");
    Rn64("0x5A110C6058B920A0");
    Rn64("0x0CD9A497658A5698");
    Rn64("0x56FD23C8F9715A4C");
    Rn64("0x284C847B9D887AAE");
    Rn64("0x04FEABFBBDB619CB");
    Rn64("0x742E1E651C60BA83");
    Rn64("0x9A9632E65904AD3C");
    Rn64("0x881B82A13B51B9E2");
    Rn64("0x506E6744CD974924");
    Rn64("0xB0183DB56FFC6A79");
    Rn64("0x0ED9B915C66ED37E");
    Rn64("0x5E11E86D5873D484");
    Rn64("0xF678647E3519AC6E");
    Rn64("0x1B85D488D0F20CC5");
    Rn64("0xDAB9FE6525D89021");
    Rn64("0x0D151D86ADB73615");
    Rn64("0xA865A54EDCC0F019");
    Rn64("0x93C42566AEF98FFB");
    Rn64("0x99E7AFEABE000731");
    Rn64("0x48CBFF086DDF285A");
    Rn64("0x7F9B6AF1EBF78BAF");
    Rn64("0x58627E1A149BBA21");
    Rn64("0x2CD16E2ABD791E33");
    Rn64("0xD363EFF5F0977996");
    Rn64("0x0CE2A38C344A6EED");
    Rn64("0x1A804AADB9CFA741");
    Rn64("0x907F30421D78C5DE");
    Rn64("0x501F65EDB3034D07");
    Rn64("0x37624AE5A48FA6E9");
    Rn64("0x957BAF61700CFF4E");
    Rn64("0x3A6C27934E31188A");
    Rn64("0xD49503536ABCA345");
    Rn64("0x088E049589C432E0");
    Rn64("0xF943AEE7FEBF21B8");
    Rn64("0x6C3B8E3E336139D3");
    Rn64("0x364F6FFA464EE52E");
    Rn64("0xD60F6DCEDC314222");
    Rn64("0x56963B0DCA418FC0");
    Rn64("0x16F50EDF91E513AF");
    Rn64("0xEF1955914B609F93");
    Rn64("0x565601C0364E3228");
    Rn64("0xECB53939887E8175");
    Rn64("0xBAC7A9A18531294B");
    Rn64("0xB344C470397BBA52");
    Rn64("0x65D34954DAF3CEBD");
    Rn64("0xB4B81B3FA97511E2");
    Rn64("0xB422061193D6F6A7");
    Rn64("0x071582401C38434D");
    Rn64("0x7A13F18BBEDC4FF5");
    Rn64("0xBC4097B116C524D2");
    Rn64("0x59B97885E2F2EA28");
    Rn64("0x99170A5DC3115544");
    Rn64("0x6F423357E7C6A9F9");
    Rn64("0x325928EE6E6F8794");
    Rn64("0xD0E4366228B03343");
    Rn64("0x565C31F7DE89EA27");
    Rn64("0x30F5611484119414");
    Rn64("0xD873DB391292ED4F");
    Rn64("0x7BD94E1D8E17DEBC");
    Rn64("0xC7D9F16864A76E94");
    Rn64("0x947AE053EE56E63C");
    Rn64("0xC8C93882F9475F5F");
    Rn64("0x3A9BF55BA91F81CA");
    Rn64("0xD9A11FBB3D9808E4");
    Rn64("0x0FD22063EDC29FCA");
    Rn64("0xB3F256D8ACA0B0B9");
    Rn64("0xB03031A8B4516E84");
    Rn64("0x35DD37D5871448AF");
    Rn64("0xE9F6082B05542E4E");
    Rn64("0xEBFAFA33D7254B59");
    Rn64("0x9255ABB50D532280");
    Rn64("0xB9AB4CE57F2D34F3");
    Rn64("0x693501D628297551");
    Rn64("0xC62C58F97DD949BF");
    Rn64("0xCD454F8F19C5126A");
    Rn64("0xBBE83F4ECC2BDECB");
    Rn64("0xDC842B7E2819E230");
    Rn64("0xBA89142E007503B8");
    Rn64("0xA3BC941D0A5061CB");
    Rn64("0xE9F6760E32CD8021");
    Rn64("0x09C7E552BC76492F");
    Rn64("0x852F54934DA55CC9");
    Rn64("0x8107FCCF064FCF56");
    Rn64("0x098954D51FFF6580");
    Rn64("0x23B70EDB1955C4BF");
    Rn64("0xC330DE426430F69D");
    Rn64("0x4715ED43E8A45C0A");
    Rn64("0xA8D7E4DAB780A08D");
    Rn64("0x0572B974F03CE0BB");
    Rn64("0xB57D2E985E1419C7");
    Rn64("0xE8D9ECBE2CF3D73F");
    Rn64("0x2FE4B17170E59750");
    Rn64("0x11317BA87905E790");
    Rn64("0x7FBF21EC8A1F45EC");
    Rn64("0x1725CABFCB045B00");
    Rn64("0x964E915CD5E2B207");
    Rn64("0x3E2B8BCBF016D66D");
    Rn64("0xBE7444E39328A0AC");
    Rn64("0xF85B2B4FBCDE44B7");
    Rn64("0x49353FEA39BA63B1");
    Rn64("0x1DD01AAFCD53486A");
    Rn64("0x1FCA8A92FD719F85");
    Rn64("0xFC7C95D827357AFA");
    Rn64("0x18A6A990C8B35EBD");
    Rn64("0xCCCB7005C6B9C28D");
    Rn64("0x3BDBB92C43B17F26");
    Rn64("0xAA70B5B4F89695A2");
    Rn64("0xE94C39A54A98307F");
    Rn64("0xB7A0B174CFF6F36E");
    Rn64("0xD4DBA84729AF48AD");
    Rn64("0x2E18BC1AD9704A68");
    Rn64("0x2DE0966DAF2F8B1C");
    Rn64("0xB9C11D5B1E43A07E");
    Rn64("0x64972D68DEE33360");
    Rn64("0x94628D38D0C20584");
    Rn64("0xDBC0D2B6AB90A559");
    Rn64("0xD2733C4335C6A72F");
    Rn64("0x7E75D99D94A70F4D");
    Rn64("0x6CED1983376FA72B");
    Rn64("0x97FCAACBF030BC24");
    Rn64("0x7B77497B32503B12");
    Rn64("0x8547EDDFB81CCB94");
    Rn64("0x79999CDFF70902CB");
    Rn64("0xCFFE1939438E9B24");
    Rn64("0x829626E3892D95D7");
    Rn64("0x92FAE24291F2B3F1");
    Rn64("0x63E22C147B9C3403");
    Rn64("0xC678B6D860284A1C");
    Rn64("0x5873888850659AE7");
    Rn64("0x0981DCD296A8736D");
    Rn64("0x9F65789A6509A440");
    Rn64("0x9FF38FED72E9052F");
    Rn64("0xE479EE5B9930578C");
    Rn64("0xE7F28ECD2D49EECD");
    Rn64("0x56C074A581EA17FE");
    Rn64("0x5544F7D774B14AEF");
    Rn64("0x7B3F0195FC6F290F");
    Rn64("0x12153635B2C0CF57");
    Rn64("0x7F5126DBBA5E0CA7");
    Rn64("0x7A76956C3EAFB413");
    Rn64("0x3D5774A11D31AB39");
    Rn64("0x8A1B083821F40CB4");
    Rn64("0x7B4A38E32537DF62");
    Rn64("0x950113646D1D6E03");
    Rn64("0x4DA8979A0041E8A9");
    Rn64("0x3BC36E078F7515D7");
    Rn64("0x5D0A12F27AD310D1");
    Rn64("0x7F9D1A2E1EBE1327");
    Rn64("0xDA3A361B1C5157B1");
    Rn64("0xDCDD7D20903D0C25");
    Rn64("0x36833336D068F707");
    Rn64("0xCE68341F79893389");
    Rn64("0xAB9090168DD05F34");
    Rn64("0x43954B3252DC25E5");
    Rn64("0xB438C2B67F98E5E9");
    Rn64("0x10DCD78E3851A492");
    Rn64("0xDBC27AB5447822BF");
    Rn64("0x9B3CDB65F82CA382");
    Rn64("0xB67B7896167B4C84");
    Rn64("0xBFCED1B0048EAC50");
    Rn64("0xA9119B60369FFEBD");
    Rn64("0x1FFF7AC80904BF45");
    Rn64("0xAC12FB171817EEE7");
    Rn64("0xAF08DA9177DDA93D");
    Rn64("0x1B0CAB936E65C744");
    Rn64("0xB559EB1D04E5E932");
    Rn64("0xC37B45B3F8D6F2BA");
    Rn64("0xC3A9DC228CAAC9E9");
    Rn64("0xF3B8B6675A6507FF");
    Rn64("0x9FC477DE4ED681DA");
    Rn64("0x67378D8ECCEF96CB");
    Rn64("0x6DD856D94D259236");
    Rn64("0xA319CE15B0B4DB31");
    Rn64("0x073973751F12DD5E");
    Rn64("0x8A8E849EB32781A5");
    Rn64("0xE1925C71285279F5");
    Rn64("0x74C04BF1790C0EFE");
    Rn64("0x4DDA48153C94938A");
    Rn64("0x9D266D6A1CC0542C");
    Rn64("0x7440FB816508C4FE");
    Rn64("0x13328503DF48229F");
    Rn64("0xD6BF7BAEE43CAC40");
    Rn64("0x4838D65F6EF6748F");
    Rn64("0x1E152328F3318DEA");
    Rn64("0x8F8419A348F296BF");
    Rn64("0x72C8834A5957B511");
    Rn64("0xD7A023A73260B45C");
    Rn64("0x94EBC8ABCFB56DAE");
    Rn64("0x9FC10D0F989993E0");
    Rn64("0xDE68A2355B93CAE6");
    Rn64("0xA44CFE79AE538BBE");
    Rn64("0x9D1D84FCCE371425");
    Rn64("0x51D2B1AB2DDFB636");
    Rn64("0x2FD7E4B9E72CD38C");
    Rn64("0x65CA5B96B7552210");
    Rn64("0xDD69A0D8AB3B546D");
    Rn64("0x604D51B25FBF70E2");
    Rn64("0x73AA8A564FB7AC9E");
    Rn64("0x1A8C1E992B941148");
    Rn64("0xAAC40A2703D9BEA0");
    Rn64("0x764DBEAE7FA4F3A6");
    Rn64("0x1E99B96E70A9BE8B");
    Rn64("0x2C5E9DEB57EF4743");
    Rn64("0x3A938FEE32D29981");
    Rn64("0x26E6DB8FFDF5ADFE");
    Rn64("0x469356C504EC9F9D");
    Rn64("0xC8763C5B08D1908C");
    Rn64("0x3F6C6AF859D80055");
    Rn64("0x7F7CC39420A3A545");
    Rn64("0x9BFB227EBDF4C5CE");
    Rn64("0x89039D79D6FC5C5C");
    Rn64("0x8FE88B57305E2AB6");
    Rn64("0xA09E8C8C35AB96DE");
    Rn64("0xFA7E393983325753");
    Rn64("0xD6B6D0ECC617C699");
    Rn64("0xDFEA21EA9E7557E3");
    Rn64("0xB67C1FA481680AF8");
    Rn64("0xCA1E3785A9E724E5");
    Rn64("0x1CFC8BED0D681639");
    Rn64("0xD18D8549D140CAEA");
    Rn64("0x4ED0FE7E9DC91335");
    Rn64("0xE4DBF0634473F5D2");
    Rn64("0x1761F93A44D5AEFE");
    Rn64("0x53898E4C3910DA55");
    Rn64("0x734DE8181F6EC39A");
    Rn64("0x2680B122BAA28D97");
    Rn64("0x298AF231C85BAFAB");
    Rn64("0x7983EED3740847D5");
    Rn64("0x66C1A2A1A60CD889");
    Rn64("0x9E17E49642A3E4C1");
    Rn64("0xEDB454E7BADC0805");
    Rn64("0x50B704CAB602C329");
    Rn64("0x4CC317FB9CDDD023");
    Rn64("0x66B4835D9EAFEA22");
    Rn64("0x219B97E26FFC81BD");
    Rn64("0x261E4E4C0A333A9D");
    Rn64("0x1FE2CCA76517DB90");
    Rn64("0xD7504DFA8816EDBB");
    Rn64("0xB9571FA04DC089C8");
    Rn64("0x1DDC0325259B27DE");
    Rn64("0xCF3F4688801EB9AA");
    Rn64("0xF4F5D05C10CAB243");
    Rn64("0x38B6525C21A42B0E");
    Rn64("0x36F60E2BA4FA6800");
    Rn64("0xEB3593803173E0CE");
    Rn64("0x9C4CD6257C5A3603");
    Rn64("0xAF0C317D32ADAA8A");
    Rn64("0x258E5A80C7204C4B");
    Rn64("0x8B889D624D44885D");
    Rn64("0xF4D14597E660F855");
    Rn64("0xD4347F66EC8941C3");
    Rn64("0xE699ED85B0DFB40D");
    Rn64("0x2472F6207C2D0484");
    Rn64("0xC2A1E7B5B459AEB5");
    Rn64("0xAB4F6451CC1D45EC");
    Rn64("0x63767572AE3D6174");
    Rn64("0xA59E0BD101731A28");
    Rn64("0x116D0016CB948F09");
    Rn64("0x2CF9C8CA052F6E9F");
    Rn64("0x0B090A7560A968E3");
    Rn64("0xABEEDDB2DDE06FF1");
    Rn64("0x58EFC10B06A2068D");
    Rn64("0xC6E57A78FBD986E0");
    Rn64("0x2EAB8CA63CE802D7");
    Rn64("0x14A195640116F336");
    Rn64("0x7C0828DD624EC390");
    Rn64("0xD74BBE77E6116AC7");
    Rn64("0x804456AF10F5FB53");
    Rn64("0xEBE9EA2ADF4321C7");
    Rn64("0x03219A39EE587A30");
    Rn64("0x49787FEF17AF9924");
    Rn64("0xA1E9300CD8520548");
    Rn64("0x5B45E522E4B1B4EF");
    Rn64("0xB49C3B3995091A36");
    Rn64("0xD4490AD526F14431");
    Rn64("0x12A8F216AF9418C2");
    Rn64("0x001F837CC7350524");
    Rn64("0x1877B51E57A764D5");
    Rn64("0xA2853B80F17F58EE");
    Rn64("0x993E1DE72D36D310");
    Rn64("0xB3598080CE64A656");
    Rn64("0x252F59CF0D9F04BB");
    Rn64("0xD23C8E176D113600");
    Rn64("0x1BDA0492E7E4586E");
    Rn64("0x21E0BD5026C619BF");
    Rn64("0x3B097ADAF088F94E");
    Rn64("0x8D14DEDB30BE846E");
    Rn64("0xF95CFFA23AF5F6F4");
    Rn64("0x3871700761B3F743");
    Rn64("0xCA672B91E9E4FA16");
    Rn64("0x64C8E531BFF53B55");
    Rn64("0x241260ED4AD1E87D");
    Rn64("0x106C09B972D2E822");
    Rn64("0x7FBA195410E5CA30");
    Rn64("0x7884D9BC6CB569D8");
    Rn64("0x0647DFEDCD894A29");
    Rn64("0x63573FF03E224774");
    Rn64("0x4FC8E9560F91B123");
    Rn64("0x1DB956E450275779");
    Rn64("0xB8D91274B9E9D4FB");
    Rn64("0xA2EBEE47E2FBFCE1");
    Rn64("0xD9F1F30CCD97FB09");
    Rn64("0xEFED53D75FD64E6B");
    Rn64("0x2E6D02C36017F67F");
    Rn64("0xA9AA4D20DB084E9B");
    Rn64("0xB64BE8D8B25396C1");
    Rn64("0x70CB6AF7C2D5BCF0");
    Rn64("0x98F076A4F7A2322E");
    Rn64("0xBF84470805E69B5F");
    Rn64("0x94C3251F06F90CF3");
    Rn64("0x3E003E616A6591E9");
    Rn64("0xB925A6CD0421AFF3");
    Rn64("0x61BDD1307C66E300");
    Rn64("0xBF8D5108E27E0D48");
    Rn64("0x240AB57A8B888B20");
    Rn64("0xFC87614BAF287E07");
    Rn64("0xEF02CDD06FFDB432");
    Rn64("0xA1082C0466DF6C0A");
    Rn64("0x8215E577001332C8");
    Rn64("0xD39BB9C3A48DB6CF");
    Rn64("0x2738259634305C14");
    Rn64("0x61CF4F94C97DF93D");
    Rn64("0x1B6BACA2AE4E125B");
    Rn64("0x758F450C88572E0B");
    Rn64("0x959F587D507A8359");
    Rn64("0xB063E962E045F54D");
    Rn64("0x60E8ED72C0DFF5D1");
    Rn64("0x7B64978555326F9F");
    Rn64("0xFD080D236DA814BA");
    Rn64("0x8C90FD9B083F4558");
    Rn64("0x106F72FE81E2C590");
    Rn64("0x7976033A39F7D952");
    Rn64("0xA4EC0132764CA04B");
    Rn64("0x733EA705FAE4FA77");
    Rn64("0xB4D8F77BC3E56167");
    Rn64("0x9E21F4F903B33FD9");
    Rn64("0x9D765E419FB69F6D");
    Rn64("0xD30C088BA61EA5EF");
    Rn64("0x5D94337FBFAF7F5B");
    Rn64("0x1A4E4822EB4D7A59");
    Rn64("0x6FFE73E81B637FB3");
    Rn64("0xDDF957BC36D8B9CA");
    Rn64("0x64D0E29EEA8838B3");
    Rn64("0x08DD9BDFD96B9F63");
    Rn64("0x087E79E5A57D1D13");
    Rn64("0xE328E230E3E2B3FB");
    Rn64("0x1C2559E30F0946BE");
    Rn64("0x720BF5F26F4D2EAA");
    Rn64("0xB0774D261CC609DB");
    Rn64("0x443F64EC5A371195");
    Rn64("0x4112CF68649A260E");
    Rn64("0xD813F2FAB7F5C5CA");
    Rn64("0x660D3257380841EE");
    Rn64("0x59AC2C7873F910A3");
    Rn64("0xE846963877671A17");
    Rn64("0x93B633ABFA3469F8");
    Rn64("0xC0C0F5A60EF4CDCF");
    Rn64("0xCAF21ECD4377B28C");
    Rn64("0x57277707199B8175");
    Rn64("0x506C11B9D90E8B1D");
    Rn64("0xD83CC2687A19255F");
    Rn64("0x4A29C6465A314CD1");
    Rn64("0xED2DF21216235097");
    Rn64("0xB5635C95FF7296E2");
    Rn64("0x22AF003AB672E811");
    Rn64("0x52E762596BF68235");
    Rn64("0x9AEBA33AC6ECC6B0");
    Rn64("0x944F6DE09134DFB6");
    Rn64("0x6C47BEC883A7DE39");
    Rn64("0x6AD047C430A12104");
    Rn64("0xA5B1CFDBA0AB4067");
    Rn64("0x7C45D833AFF07862");
    Rn64("0x5092EF950A16DA0B");
    Rn64("0x9338E69C052B8E7B");
    Rn64("0x455A4B4CFE30E3F5");
    Rn64("0x6B02E63195AD0CF8");
    Rn64("0x6B17B224BAD6BF27");
    Rn64("0xD1E0CCD25BB9C169");
    Rn64("0xDE0C89A556B9AE70");
    Rn64("0x50065E535A213CF6");
    Rn64("0x9C1169FA2777B874");
    Rn64("0x78EDEFD694AF1EED");
    Rn64("0x6DC93D9526A50E68");
    Rn64("0xEE97F453F06791ED");
    Rn64("0x32AB0EDB696703D3");
    Rn64("0x3A6853C7E70757A7");
    Rn64("0x31865CED6120F37D");
    Rn64("0x67FEF95D92607890");
    Rn64("0x1F2B1D1F15F6DC9C");
    Rn64("0xB69E38A8965C6B65");
    Rn64("0xAA9119FF184CCCF4");
    Rn64("0xF43C732873F24C13");
    Rn64("0xFB4A3D794A9A80D2");
    Rn64("0x3550C2321FD6109C");
    Rn64("0x371F77E76BB8417E");
    Rn64("0x6BFA9AAE5EC05779");
    Rn64("0xCD04F3FF001A4778");
    Rn64("0xE3273522064480CA");
    Rn64("0x9F91508BFFCFC14A");
    Rn64("0x049A7F41061A9E60");
    Rn64("0xFCB6BE43A9F2FE9B");
    Rn64("0x08DE8A1C7797DA9B");
    Rn64("0x8F9887E6078735A1");
    Rn64("0xB5B4071DBFC73A66");
    Rn64("0x230E343DFBA08D33");
    Rn64("0x43ED7F5A0FAE657D");
    Rn64("0x3A88A0FBBCB05C63");
    Rn64("0x21874B8B4D2DBC4F");
    Rn64("0x1BDEA12E35F6A8C9");
    Rn64("0x53C065C6C8E63528");
    Rn64("0xE34A1D250E7A8D6B");
    Rn64("0xD6B04D3B7651DD7E");
    Rn64("0x5E90277E7CB39E2D");
    Rn64("0x2C046F22062DC67D");
    Rn64("0xB10BB459132D0A26");
    Rn64("0x3FA9DDFB67E2F199");
    Rn64("0x0E09B88E1914F7AF");
    Rn64("0x10E8B35AF3EEAB37");
    Rn64("0x9EEDECA8E272B933");
    Rn64("0xD4C718BC4AE8AE5F");
    Rn64("0x81536D601170FC20");
    Rn64("0x91B534F885818A06");
    Rn64("0xEC8177F83F900978");
    Rn64("0x190E714FADA5156E");
    Rn64("0xB592BF39B0364963");
    Rn64("0x89C350C893AE7DC1");
    Rn64("0xAC042E70F8B383F2");
    Rn64("0xB49B52E587A1EE60");
    Rn64("0xFB152FE3FF26DA89");
    Rn64("0x3E666E6F69AE2C15");
    Rn64("0x3B544EBE544C19F9");
    Rn64("0xE805A1E290CF2456");
    Rn64("0x24B33C9D7ED25117");
    Rn64("0xE74733427B72F0C1");
    Rn64("0x0A804D18B7097475");
    Rn64("0x57E3306D881EDB4F");
    Rn64("0x4AE7D6A36EB5DBCB");
    Rn64("0x2D8D5432157064C8");
    Rn64("0xD1E649DE1E7F268B");
    Rn64("0x8A328A1CEDFE552C");
    Rn64("0x07A3AEC79624C7DA");
    Rn64("0x84547DDC3E203C94");
    Rn64("0x990A98FD5071D263");
    Rn64("0x1A4FF12616EEFC89");
    Rn64("0xF6F7FD1431714200");
    Rn64("0x30C05B1BA332F41C");
    Rn64("0x8D2636B81555A786");
    Rn64("0x46C9FEB55D120902");
    Rn64("0xCCEC0A73B49C9921");
    Rn64("0x4E9D2827355FC492");
    Rn64("0x19EBB029435DCB0F");
    Rn64("0x4659D2B743848A2C");
    Rn64("0x963EF2C96B33BE31");
    Rn64("0x74F85198B05A2E7D");
    Rn64("0x5A0F544DD2B1FB18");
    Rn64("0x03727073C2E134B1");
    Rn64("0xC7F6AA2DE59AEA61");
    Rn64("0x352787BAA0D7C22F");
    Rn64("0x9853EAB63B5E0B35");
    Rn64("0xABBDCDD7ED5C0860");
    Rn64("0xCF05DAF5AC8D77B0");
    Rn64("0x49CAD48CEBF4A71E");
    Rn64("0x7A4C10EC2158C4A6");
    Rn64("0xD9E92AA246BF719E");
    Rn64("0x13AE978D09FE5557");
    Rn64("0x730499AF921549FF");
    Rn64("0x4E4B705B92903BA4");
    Rn64("0xFF577222C14F0A3A");
    Rn64("0x55B6344CF97AAFAE");
    Rn64("0xB862225B055B6960");
    Rn64("0xCAC09AFBDDD2CDB4");
    Rn64("0xDAF8E9829FE96B5F");
    Rn64("0xB5FDFC5D3132C498");
    Rn64("0x310CB380DB6F7503");
    Rn64("0xE87FBB46217A360E");
    Rn64("0x2102AE466EBB1148");
    Rn64("0xF8549E1A3AA5E00D");
    Rn64("0x07A69AFDCC42261A");
    Rn64("0xC4C118BFE78FEAAE");
    Rn64("0xF9F4892ED96BD438");
    Rn64("0x1AF3DBE25D8F45DA");
    Rn64("0xF5B4B0B0D2DEEEB4");
    Rn64("0x962ACEEFA82E1C84");
    Rn64("0x046E3ECAAF453CE9");
    Rn64("0xF05D129681949A4C");
    Rn64("0x964781CE734B3C84");
    Rn64("0x9C2ED44081CE5FBD");
    Rn64("0x522E23F3925E319E");
    Rn64("0x177E00F9FC32F791");
    Rn64("0x2BC60A63A6F3B3F2");
    Rn64("0x222BBFAE61725606");
    Rn64("0x486289DDCC3D6780");
    Rn64("0x7DC7785B8EFDFC80");
    Rn64("0x8AF38731C02BA980");
    Rn64("0x1FAB64EA29A2DDF7");
    Rn64("0xE4D9429322CD065A");
    Rn64("0x9DA058C67844F20C");
    Rn64("0x24C0E332B70019B0");
    Rn64("0x233003B5A6CFE6AD");
    Rn64("0xD586BD01C5C217F6");
    Rn64("0x5E5637885F29BC2B");
    Rn64("0x7EBA726D8C94094B");
    Rn64("0x0A56A5F0BFE39272");
    Rn64("0xD79476A84EE20D06");
    Rn64("0x9E4C1269BAA4BF37");
    Rn64("0x17EFEE45B0DEE640");
    Rn64("0x1D95B0A5FCF90BC6");
    Rn64("0x93CBE0B699C2585D");
    Rn64("0x65FA4F227A2B6D79");
    Rn64("0xD5F9E858292504D5");
    Rn64("0xC2B5A03F71471A6F");
    Rn64("0x59300222B4561E00");
    Rn64("0xCE2F8642CA0712DC");
    Rn64("0x7CA9723FBB2E8988");
    Rn64("0x2785338347F2BA08");
    Rn64("0xC61BB3A141E50E8C");
    Rn64("0x150F361DAB9DEC26");
    Rn64("0x9F6A419D382595F4");
    Rn64("0x64A53DC924FE7AC9");
    Rn64("0x142DE49FFF7A7C3D");
    Rn64("0x0C335248857FA9E7");
    Rn64("0x0A9C32D5EAE45305");
    Rn64("0xE6C42178C4BBB92E");
    Rn64("0x71F1CE2490D20B07");
    Rn64("0xF1BCC3D275AFE51A");
    Rn64("0xE728E8C83C334074");
    Rn64("0x96FBF83A12884624");
    Rn64("0x81A1549FD6573DA5");
    Rn64("0x5FA7867CAF35E149");
    Rn64("0x56986E2EF3ED091B");
    Rn64("0x917F1DD5F8886C61");
    Rn64("0xD20D8C88C8FFE65F");
    Rn64("0x31D71DCE64B2C310");
    Rn64("0xF165B587DF898190");
    Rn64("0xA57E6339DD2CF3A0");
    Rn64("0x1EF6E6DBB1961EC9");
    Rn64("0x70CC73D90BC26E24");
    Rn64("0xE21A6B35DF0C3AD7");
    Rn64("0x003A93D8B2806962");
    Rn64("0x1C99DED33CB890A1");
    Rn64("0xCF3145DE0ADD4289");
    Rn64("0xD0E4427A5514FB72");
    Rn64("0x77C621CC9FB3A483");
    Rn64("0x67A34DAC4356550B");
...
function kbpk_draw(wp, wb, bk) -- bool
    ...
    if (SQUARE_FILE(wp) == FileA and DISTANCE(bk, A8) <= 1 and SQUARE_COLOUR(wb) ~= SQUARE_COLOUR(A8)) then
        return true;
    end
...
function search() -- void
    ...
    while (true) do -- setjmp loop
        ...
        for depth = 1, DepthMax - 1, 1 do
            ...
            if (depth == 1 and SearchRoot.list.size >= 2 and SearchRoot.list.value[1 + 0] >=
                SearchRoot.list.value[1 + 1] + EasyThreshold) then
            ...
            if (SearchInput.depth_is_limited and depth >= SearchInput.depth_limit) then
                SearchRoot.flag = true;
            end

            if (SearchInput.time_is_limited and SearchCurrent.time >= SearchInput.time_limit_1 and not SearchRoot.bad_2) then
                SearchRoot.flag = true;
            end

            if (UseEasy and SearchInput.time_is_limited and SearchCurrent.time >= SearchInput.time_limit_1 * EasyRatio and
                SearchRoot.easy) then
                ASSERT(638, not SearchRoot.bad_2);
                ASSERT(639, not SearchRoot.change);
                SearchRoot.flag = true;
            end

            if (UseEarly and SearchInput.time_is_limited and SearchCurrent.time >= SearchInput.time_limit_1 * EarlyRatio and
                not SearchRoot.bad_2 and not SearchRoot.change) then
                SearchRoot.flag = true;
            end

            if (SearchInfo.can_stop and (SearchInfo.stop or (SearchRoot.flag and not SearchInput.infinite))) then
                return;
            end
        end
...
function search_check() -- void
    ...
    if (SearchInput.depth_is_limited and SearchRoot.depth > SearchInput.depth_limit) then
        SearchRoot.flag = true;
    end

    if (SearchInput.time_is_limited and SearchCurrent.time >= SearchInput.time_limit_2) then
        SearchRoot.flag = true;
    end

    if (SearchInput.time_is_limited and SearchCurrent.time >= SearchInput.time_limit_1 and not SearchRoot.bad_1 and
        not SearchRoot.bad_2 and (not UseExtension or SearchRoot.move_pos == 0)) then
        SearchRoot.flag = true;
    end
...
function full_search(board, alpha, beta, depth, height, pv, node_type) -- int
    ...
    if (Usenil and depth >= nilDepth and node_type ~= NodePV) then
        ...
        if (not in_check and not value_is_mate(beta) and do_nil(board) and
            (not UsenilEval or depth <= nilReduction + 1 or evalpos(board) >= beta)) then
    ...
    if (not gotocut) then -- [1]
        ...
        while (true) do
            ...
            if (UseFutility and depth == 1 and node_type ~= NodePV) then
                ...
                if ((not in_check) and new_depth == 0 and (not move_is_tactical(move, board)) and
                    (not move_is_dangerous(move, board))) then
...
function full_new_depth(depth, move, board, single_reply, in_pv) -- int
    ...
    b = b or (in_pv and PIECE_IS_PAWN(MOVE_PIECE(move, board)) and PAWN_RANK(MOVE_TO(move), board.turn) == Rank7 and
...
function my_timer_start(timer)
    ...
    timer.running = true;
    timer.start_real = playdate.getCurrentTimeMilliseconds();
...
function my_timer_stop(timer)
    ...
    timer.elapsed_real = timer.elapsed_real + playdate.getCurrentTimeMilliseconds() - timer.start_real;
    timer.start_real = 0.0;
    timer.running = false;
...
function my_timer_elapsed_real(timer) -- int
    ...
    if (timer.running) then
        timer.elapsed_real = (playdate.getCurrentTimeMilliseconds() - timer.start_real);
    end
...
function main()
    ...
    print("value_init")
    vector_init();
    print("vector_init") -- works up to here
    attack_init();
    print("attack_init")
    -- move_do_init();
    -- print("move_init")

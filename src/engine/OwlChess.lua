...
MAXPLY = 1;		-- max.ply
MAXSECS = 1;		-- max.seconds to search
...
function InitTime()
 timer.started = playdate.getCurrentTimeMilliseconds()
 timer.elapsed = 0;
end
InitTime();
...
function timeused()
 ...
 if(Analysis) then
    ...
    timer.elapsed = math.floor( playdate.getCurrentTimeMilliseconds() - timer.started );
...
function FindOpeningMove()
    ...
    local r = math.floor(playdate.getCurrentTimeMilliseconds()*math.random()*16) % 16;
